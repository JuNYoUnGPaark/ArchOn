...
def reset_runtime_for_new_session():
    global current_session_id, current_jsonl_path, all_samples, exercise_results, current_exercise_samples, exercise_started_at
    current_session_id = None
    current_jsonl_path = None
    all_samples = []
    exercise_results = []
...
def mvic_task():
    ...
    try:
        ...
        samples = collect_samples(seconds, f"mvic_{current_trial}", f"아치를 유지하면서 엄지발가락으로 바닥을 최대한 강하게 누르세요. MVIC {current_trial}회차")
...
def exercise_loop():
    ...
    while phase == "exercising":
        ...
        socketio.emit("sensor_data", {**sample, "feedback": "운동 중입니다."})
